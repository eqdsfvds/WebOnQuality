!function(e, t) {
    ...
    return function(e) {
        ...
        class i {
            ...
            addNewNode(name){
                var ele_selected = this.container.getElementsByClassName("selected")[0];
                var html = "<div style='text-align:center'><label style='cursor: move;outline: none'>"+name+"</label></div>";
                var conn = this.getNodeFromId(ele_selected.id.slice(5)).outputs.output_1.connections;
                var startVert = ele_selected.offsetLeft;
                var startHorz = ele_selected.offsetTop - conn.length/2 * 100;
                for(var i = 0; i < conn.length; i++){
                    var x = document.getElementById("node-" + conn[i]["node"]);
                    x.style.top = startHorz + 100 * i +"px";
                    x.style.left = startVert + 300 + "px";
                    this.updateConnectionNodes("node-" + conn[i]["node"]);
                }
                var id = this.addNode('AAA', 1, 1, startVert + 300, startHorz + 100 * i, '', {}, html);
                this.addConnection(ele_selected.id.slice(5), id, 'output_1','input_1');
                this.node_selected.style.zIndex=2;
            }
            addEditEvent(){
                $("body").on("dblclick",".drawflow-node",function(e){
                    var la = $(this).find("label")[0]
                    $(la).css("cursor", "text")
                    // 添加name标签用以比对是否改变了值
                    $(la).attr("name", $(la).text())
                    $(la).attr("contentEditable", true)
                    la.focus()
                ...
                $("body").on("focusout",".drawflow-node" ,function(e){
                    var la = $(this).find("label")[0]
                    $(la).css("cursor", "move")
                    $(la).attr("contentEditable", false)
                    // 结点名称是否改变
                    if($(la).text() != $(la).attr("name")){
                        //更新至数据库
                    }
                });
            }
            ...
            click(e) {
                if(e.target.tagName == "LABEL" && $(e.target).attr("contenteditable") == "true") return !1
                if (this.dispatch("click", e),
                "fixed" === this.editor_mode) {
                    if (e.preventDefault(),
                    "parent-drawflow" !== e.target.classList[0] && "drawflow" !== e.target.classList[0])
                        return !1;
                    this.ele_selected = e.target.closest(".parent-drawflow")
                } else
                    "view" === this.editor_mode ? (null != e.target.closest(".drawflow") || e.target.matches(".parent-drawflow")) && (this.ele_selected = e.target.closest(".parent-drawflow"),
                    e.preventDefault()) : (this.first_click = e.target,
                ...
                case "drawflow-node":
                    ...
                    break;
				case "drawflow-addSub":
                    $(".modalWin")[0].classList.remove("hide")
                    $(".modalWin")[0].setAttribute("name", "Sub")
                    $(".shade")[0].classList.remove("hide")
                    $("#flow-name").val("")
                    $(".multi-check").empty()
                    var multiCheckArea = $(".multi-check")[0];
                    $.post('http://11.11.1.133:8888/data',{
                        target: "getFuncFirstFlow"
                        })
                        .success(function(data){
                            var d = $.parseJSON(data).r
                            var maxLength = d.length
                            for(var i = 0; i < maxLength; i++){
                                var tmp = d[i]
                                var row = $("<div class='form-check'></div>")[0]
                                row.innerHTML = '<input class="form-check-input" type="checkbox" value="'+tmp["key"]+'" id="function-'+i+'">'+
                                '<label style="padding: 10px" class="form-check-label" for="function-'+i+'">'+
                                tmp["funcName"] + '</label>'
                                multiCheckArea.appendChild(row)
                            }
                        })
                        .error(function(data){
                            alert("error")
					...
					break;
                case "drawflow-addSecSub":
				//this.addNewNode("Node");
                    $(".modalWin")[0].classList.remove("hide")
                    $(".modalWin")[0].setAttribute("name", "SecSub")
                    $(".shade")[0].classList.remove("hide")
                    $("#flow-name").val("")
                    $(".multi-check").empty()
                    var multiCheckArea = $(".multi-check")[0];
                    $.post('http://11.11.1.133:8888/data',{
                        target: "getFuncSecFlow"
                        })
                        .success(function(data){
                            var d = $.parseJSON(data).r
                            var maxLength = d.length
                ...
                this.mouse_y = e.touches[0].clientY) : (this.pos_x = e.clientX,
            ...
            position(e) {
                ...
                this.editor_selected && (i = this.canvas_x + -(this.pos_x - t),
            ...
            dragEnd(e) {
                ...
                this.pos_x_start == t && this.pos_y_start == n || this.dispatch("rerouteMoved", this.ele_selected.parentElement.classList[2].slice(14))),
                ...
                !0 === this.connection)
                    if ("input" === i.classList[0] || this.force_first_input && (null != i.closest(".drawflow_content_node") || "drawflow-node" === i.classList[0])) {
                        if (!this.force_first_input || null == i.closest(".drawflow_content_node") && "drawflow-node" !== i.classList[0])
                            s = i.parentElement.parentElement.id,
                            o = i.classList[1];
                        else {
            ...
            contextmenu(e) {
                if (this.dispatch("contextmenu", e),
					e.preventDefault(),
					"fixed" === this.this_mode || "view" === this.this_mode)
						return !1;
				if (this.precanvas.getElementsByClassName("drawflow-delete").length && this.contextmenuDel(),
					this.node_selected || this.connection_selected) {
						this.node_selected.style.zIndex = 3;
						// 测试
						var t = document.createElement("div");
						t.classList.add("drawflow-test"),
                        t.classList.add("drawflow-options"),
						t.innerHTML = "测试",
						t.style.top = "-10px",
						t.style.right = "214px",
						...
						/* 连接线选择时弹出选项（暂时不用）
						this.connection_selected && this.connection_selected.parentElement.classList.length > 1 && (t.style.top = e.clientY * (this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom)) - this.precanvas.getBoundingClientRect().y * (this.precanvas.clientHeight / (this.precanvas.clientHeight * this.zoom)) + "px",
						...
						*/
						...
						// 过程项
						var t = document.createElement("div");
						t.classList.add("drawflow-addSub"),
                        t.classList.add("drawflow-options"),
						t.innerHTML = "添加工序",
						t.style.top = "-75px",
						t.style.right = "-46px",
						...
						// 子工序
						var t = document.createElement("div");
						t.classList.add("drawflow-addSecSub"),
                        t.classList.add("drawflow-options"),
						t.setAttribute("data-bs-toggle", "modal"),
						t.setAttribute("data-bs-target", "#modalAddNode"),
						t.innerHTML = "添加子工序",
						t.style.top = "-10px",
						t.style.right = "-114px",
						t.style.position = "absolute",
            ...
            contextmenuDel() {
				Array.from(this.precanvas.getElementsByClassName("drawflow-delete")).forEach(function(element){element.remove();});
				Array.from(this.precanvas.getElementsByClassName("drawflow-addSub")).forEach(function(element){element.remove();});
                this.precanvas.getElementsByClassName("drawflow-addSecSub").length && this.precanvas.getElementsByClassName("drawflow-addSecSub")[0].remove();
				this.precanvas.getElementsByClassName("drawflow-test").length && this.precanvas.getElementsByClassName("drawflow-test")[0].remove();
            }
            key(e) {
            ...
            createReroutePoint(e) {
                ...
                this.reroute_fix_curvature ? (d > 0 || this.drawflow.drawflow[this.module].data[r].outputs[i].connections[h].points !== [] ? this.drawflow.drawflow[this.module].data[r].outputs[i].connections[h].points.splice(d, 0, {
                    pos_x: l,
                    pos_y: c
            ...
            addNode(e, t, n, i, s, o, l, c, d=!1) {
                ...
                !1 === d)
                    ...
                    e.appContext = this.parent,
                    this.render.render(e, _)
                } else {
            ...
            addNodeImport(e, t) {
                ...
                !1 === e.typenode)
                    ...
                    t.appContext = this.parent,
                    this.render.render(t, c)
                } else {
            ...
            removeSingleConnection(e, t, n, i) {
                ...
                if (s === this.getModuleFromNodeId(t)) {
                    if (this.drawflow.drawflow[s].data[e].outputs[n].connections.findIndex((function(e, n) {
                        ...
                        return this.drawflow.drawflow[s].data[t].inputs[i].connections.splice(l, 1),
                        this.dispatch("connectionRemoved", {
                            output_id: e,
                            input_id: t,
                            output_class: n,
